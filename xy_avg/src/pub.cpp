...
ros::Publisher chatter_pub = n.advertise<xy_avg::Num>("chatter", 1000);
...
//generate two x and y data points from 0 to 100, store in an object



data.x = (std::rand() % 100 + (std::rand()/((float)RAND_MAX+1)));
data.y = (std::rand() % 100 + (std::rand()/((float)RAND_MAX+1)));

data.x2 = (std::rand() % 100 + (std::rand()/((float)RAND_MAX+1)));
data.y2 = (std::rand() % 100 + (std::rand()/((float)RAND_MAX+1)));



//automatically time stamped printf function
ROS_INFO("Point 1 is: x-[%f] and y-[%f]",data.x, data.y);
ROS_INFO("Point 2 is: x-[%f] and y-[%f] \n",data.x2, data.y2);
